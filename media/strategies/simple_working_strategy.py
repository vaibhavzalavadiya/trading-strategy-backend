...
def run_strategy(df):
    """
    Simple Working Strategy - Always generates results
    This strategy will work with any data format and generate backtest results
    """
    ...
    # Find the close price column (handle different naming)
    close_col = None
    ...
    df['Signal'] = 0
    df.loc[df['MA10'] > df['MA20'], 'Signal'] = 1  # Buy when short MA > long MA
    df.loc[df['MA10'] < df['MA20'], 'Signal'] = -1  # Sell when short MA < long MA
    ...
    for i in range(20, len(df)):  # Start from 20 to ensure MA20 is calculated
