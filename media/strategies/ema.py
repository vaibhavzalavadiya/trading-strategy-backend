...
def run_ema5_buy_strategy(df, capital=100000, risk_pct=0.10, stop_loss_pct=0.02, target_pct=0.10):
    """
    Entry Conditions:
    - Previous candle closes fully below 5 EMA (high < EMA and close < EMA)
    - Current candle is green (close > open)
    - Price is NOT in uptrend (current close < EMA 21)
    
    Exit:
    - Stop Loss hit at 2%
    - Take Profit hit at 10% (1:5 RR)
    """
